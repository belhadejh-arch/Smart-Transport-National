    ...
    <View style={s.screen}>
      ...
      <View style={s.content}>
        ...
        {isLoading ? (
          ...
          <>
            <View style={[s.cardVisual, { backgroundColor: cardTypeColors[card.type] ?? C.primary }]}>
              <View style={s.cardTop}>
                <Text style={s.cardAppName}>NQL DZ</Text>
                <Text style={s.cardEmoji}>{cardEmojis[card.type] ?? "🚌"}</Text>
              </View>
              ...
              <View style={s.cardBottom}>
                <View>
                  <Text style={s.cardBalanceLabel}>{t.common.balance}</Text>
                  <Text style={s.cardBalance}>{Number(card.balance).toFixed(0)} {t.common.dinar}</Text>
                </View>
                <View>
                  <Text style={s.cardFareLabel}>Fare</Text>
                  <Text style={s.cardFare}>{fares[card.type] ?? "50"} {t.common.dinar}</Text>
                </View>
              </View>
              <View style={[s.statusPill, { backgroundColor: card.status === "active" ? "rgba(255,255,255,0.25)" : "rgba(255,0,0,0.3)" }]}>
            ...
            <View style={s.actionsRow}>
              <TouchableOpacity style={s.actionTile} onPress={() => router.push("/(customer)/my-card")} activeOpacity={0.8}>
                <Text style={s.actionTileIcon}>📲</Text>
                <Text style={s.actionTileLabel}>{t.customer.showQR}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.actionTile} onPress={() => router.push("/(customer)/transactions")} activeOpacity={0.8}>
                <Text style={s.actionTileIcon}>📋</Text>
                <Text style={s.actionTileLabel}>{t.customer.transactions}</Text>
              </TouchableOpacity>
            </View>
          </>
        ) : (
          <View style={s.noCard}>
            ...
            <TouchableOpacity style={s.createCardBtn} onPress={() => router.push("/(customer)/create-card")} activeOpacity={0.85}>
              <Text style={s.createCardBtnText}>+ {t.customer.createCard}</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
